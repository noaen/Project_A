...
,	vector<float> &img_blurred
    ...
    if (nb_threads == 1)
    {
        ...
        //! Denoising, 1st Step
        cout << "step 1...";
        //exporting img so we can use it to draw the matched blocks
        char* filename = "img_for_BM.png";
        save_image(filename, img_sym_noisy, w_b, h_b, chnls);
...
,   const float    tauMatch
    ...
    //exporting the patch table to csv
    std::ofstream out("patch_table.csv");
    for (unsigned int row = 0; row<patch_table.size(); row++) {
    	for (unsigned int col = 0; col< patch_table[row].size(); col++){
    		out << patch_table[row][col] <<',';
    		//out<<col<<',';
    	}
    	out << '\n';
    }
    ...
    //exporting the image table to csv
    std::ofstream out_im("image.csv");
    for (unsigned int idx = 0; idx<img.size(); idx++) {
    	out_im << img[idx] <<',';
    		//out<<col<<',';
    }
    out_im << std::endl;
